...
[​](https://docs.langchain.com/oss/python/deepagents/skills#usage) Usage
------------------------------------------------------------------------
...
The `SKILL.md` starts with YAML [frontmatter](https://docs.langchain.com/oss/python/deepagents/skills#frontmatter-fields) followed by markdown instructions:
...
Reference any [supporting resources](https://docs.langchain.com/oss/python/deepagents/skills#add-supporting-resources) in your `SKILL.md` with a description of what each file contains and when to use it. The agent discovers these files through the references in the skill instructions.
...
This example uses `FilesystemBackend` to load skills from disk. For other storage options, including loading skills from remote sources, see [Backends and remote skill loading](https://docs.langchain.com/oss/python/deepagents/skills#backends-and-remote-skill-loading).

[​](https://docs.langchain.com/oss/python/deepagents/skills#param-skills)
...
Send a task to the agent with `invoke()`. At startup, the agent loads each skill’s [`name`](https://docs.langchain.com/oss/python/deepagents/skills#frontmatter-fields) and [`description`](https://docs.langchain.com/oss/python/deepagents/skills#frontmatter-fields) from [frontmatter](https://docs.langchain.com/oss/python/deepagents/skills#frontmatter-fields) into the system prompt. When your task matches a skill’s description, the agent reads that skill’s `SKILL.md` and follows its instructions.
...
[​](https://docs.langchain.com/oss/python/deepagents/skills#how-skills-work) How skills work
--------------------------------------------------------------------------------------------
...
| --- | --- | --- |
| **1. Metadata** | [`name`](https://docs.langchain.com/oss/python/deepagents/skills#frontmatter-fields) and [`description`](https://docs.langchain.com/oss/python/deepagents/skills#frontmatter-fields) from `SKILL.md` [frontmatter](https://docs.langchain.com/oss/python/deepagents/skills#frontmatter-fields) | Agent startup, for every configured skill |
| **2. Instructions** | Full `SKILL.md` body | When the skill is invoked |
| **3. Resources** | [Supporting files](https://docs.langchain.com/oss/python/deepagents/skills#add-supporting-resources) under `scripts/`, `references/`, and `assets/` | As needed after invocation, when the instructions reference them |
...
In Deep Agents, [`SkillsMiddleware`](https://reference.langchain.com/python/deepagents/middleware/skills/SkillsMiddleware) (part of the [default middleware stack](https://docs.langchain.com/oss/python/deepagents/customization#default-stack-main-agent) when you pass `skills`) handles the first two levels, with the third level being handled by the LLM:

1. **Discovery** (level 1): At agent start, the middleware scans the configured skill paths, parses each `SKILL.md` [frontmatter](https://docs.langchain.com/oss/python/deepagents/skills#frontmatter-fields), and injects the [`name`](https://docs.langchain.com/oss/python/deepagents/skills#frontmatter-fields) and [`description`](https://docs.langchain.com/oss/python/deepagents/skills#frontmatter-fields) fields into the system prompt.
...
[​](https://docs.langchain.com/oss/python/deepagents/skills#when-to-use-skills) When to use skills
--------------------------------------------------------------------------------------------------
...
* **Instructions with executable code**: Bundle procedures with scripts or modules the agent can run, so it follows tested logic instead of regenerating it from instructions each time. See [Execute code with skills](https://docs.langchain.com/oss/python/deepagents/skills#execute-code-with-skills).
...
[​](https://docs.langchain.com/oss/python/deepagents/skills#write-effective-skills) Write effective skills
----------------------------------------------------------------------------------------------------------
...
**Keep [frontmatter](https://docs.langchain.com/oss/python/deepagents/skills#frontmatter-fields) concise** and the `SKILL.md` body under 5,000 tokens. Every skill’s frontmatter is added to the system prompt at [discovery](https://docs.langchain.com/oss/python/deepagents/skills#how-skills-work), while the full body is only read when activated. Keeping both layers small means you can load many skills without crowding the context window.
**Write specific descriptions.** During [discovery](https://docs.langchain.com/oss/python/deepagents/skills#how-skills-work), the [`description`](https://docs.langchain.com/oss/python/deepagents/skills#frontmatter-fields) field is the only information the agent sees for each skill. A good description tells the agent both what the skill does and when to activate it, with specific keywords the agent can match against:
...
**Keep instructions focused.** The Agent Skills specification recommends keeping your `SKILL.md` under 500 lines. When instructions grow longer, move detailed reference material into [supporting resource files](https://docs.langchain.com/oss/python/deepagents/skills#add-supporting-resources) and reference them from the main `SKILL.md`:
...
Use the [`skills-ref` validation tool](https://github.com/agentskills/agentskills/tree/main/skills-ref) to check that your `SKILL.md` [frontmatter](https://docs.langchain.com/oss/python/deepagents/skills#frontmatter-fields) follows the Agent Skills specification naming and format conventions.

[​](https://docs.langchain.com/oss/python/deepagents/skills#add-supporting-resources) Add supporting resources
--------------------------------------------------------------------------------------------------------------
...
### [​](https://docs.langchain.com/oss/python/deepagents/skills#scripts/) `scripts/`
...
Supported languages depend on your agent setup. Common options include Python, Bash, and JavaScript or TypeScript. To execute scripts rather than only read them, see [Execute code with skills](https://docs.langchain.com/oss/python/deepagents/skills#execute-code-with-skills). Use [sandbox scripts](https://docs.langchain.com/oss/python/deepagents/skills#sandbox-scripts) when the agent needs a shell.

### [​](https://docs.langchain.com/oss/python/deepagents/skills#references/) `references/`
...
### [​](https://docs.langchain.com/oss/python/deepagents/skills#assets/) `assets/`
...
### [​](https://docs.langchain.com/oss/python/deepagents/skills#reference-files-from-skill-md) Reference files from `SKILL.md`
...
[​](https://docs.langchain.com/oss/python/deepagents/skills#backends-and-remote-skill-loading) Backends and remote skill loading
--------------------------------------------------------------------------------------------------------------------------------
...
[​](https://docs.langchain.com/oss/python/deepagents/skills#load-skills-at-runtime) Load skills at runtime
----------------------------------------------------------------------------------------------------------
...
### [​](https://docs.langchain.com/oss/python/deepagents/skills#dynamic-skill-lists) Dynamic skill lists
...
The SDK only loads the sources you pass in `skills`. It does not automatically scan CLI directories such as `~/.deepagents/...` or `~/.agents/...`.For CLI storage conventions, see [App data](https://docs.langchain.com/oss/python/deepagents/code/data-locations).
...
### [​](https://docs.langchain.com/oss/python/deepagents/skills#namespaced-skills) Namespaced skills
...
This pattern is useful when different users or tenants need fully independent skill libraries that can be updated separately. For a managed solution that handles skill access, sharing, and workspace-level visibility out of the box, see [Fleet skills](https://docs.langchain.com/langsmith/fleet/skills).

[​](https://docs.langchain.com/oss/python/deepagents/skills#skills-for-subagents) Skills for subagents
------------------------------------------------------------------------------------------------------

When you use [subagents](https://docs.langchain.com/oss/python/deepagents/subagents), you can configure which skills each type has access to:
...
For more information on subagent configuration and skills inheritance, see [Subagents](https://docs.langchain.com/oss/python/deepagents/subagents).

[​](https://docs.langchain.com/oss/python/deepagents/skills#skill-permissions) Skill permissions
------------------------------------------------------------------------------------------------

Production deployments usually need to control three things: which skills each user can see, whether the agent can modify skill files, and whether writes require human approval. You control visibility with the `skills` argument and [backend routing](https://docs.langchain.com/oss/python/deepagents/skills#backends-and-remote-skill-loading), access with [filesystem permissions](https://docs.langchain.com/oss/python/deepagents/permissions), and approval with [`interrupt_on`](https://docs.langchain.com/oss/python/deepagents/human-in-the-loop) or permission rules with `mode="interrupt"`.

### [​](https://docs.langchain.com/oss/python/deepagents/skills#share-skills-across-users) Share skills across users
...
* Namespace by org ID for workspace-wide skills (see [Enforce read-only skills](https://docs.langchain.com/oss/python/deepagents/skills#enforce-read-only-skills)).
* Namespace by user ID when each user needs an independent library ([namespaced skills](https://docs.langchain.com/oss/python/deepagents/skills#namespaced-skills)).
...
For a managed solution that handles skill access, sharing, and workspace-level visibility, see [Fleet skills](https://docs.langchain.com/langsmith/fleet/skills).
You can also combine shared and personal libraries: route `/skills/shared/` to an organization-scoped `StoreBackend`, route `/skills/personal/` to a user-scoped backend, and pass both paths in `skills`. See [Allow agents to edit personal skills](https://docs.langchain.com/oss/python/deepagents/skills#allow-agents-to-edit-personal-skills).

### [​](https://docs.langchain.com/oss/python/deepagents/skills#limit-skills-by-user-context) Limit skills by user context
...
* **[Dynamic skill lists](https://docs.langchain.com/oss/python/deepagents/skills#dynamic-skill-lists)** — Build the `skills` array before creating the agent. Pass different path lists for different roles or request types. Works when skills live in a shared backend and you filter by path.
* **[Namespaced skills](https://docs.langchain.com/oss/python/deepagents/skills#namespaced-skills)** — Route `/skills/` to a `StoreBackend` with a namespace factory keyed on user or tenant ID. Populate each namespace with only the skills that identity should access.
...
### [​](https://docs.langchain.com/oss/python/deepagents/skills#enforce-read-only-skills) Enforce read-only skills

To share skills without letting agents modify them, route `/skills/` to a shared store and deny write operations under `/skills/**` with [filesystem permissions](https://docs.langchain.com/oss/python/deepagents/permissions). The agent can discover and read skills; only your application code or an admin workflow updates the store.
...
### [​](https://docs.langchain.com/oss/python/deepagents/skills#require-approval-for-skill-writes) Require approval for skill writes

If agents may write to skill files but you want a human in the loop first, use either [`interrupt_on`](https://docs.langchain.com/oss/python/deepagents/human-in-the-loop) or a permission rule with `mode="interrupt"`. Both pause before `write_file` or `edit_file` runs and use the same resume flow.
...
Alternatively, configure `interrupt_on={"write_file": True, "edit_file": True}` to require approval for all filesystem writes, not only skills paths. See [Human-in-the-loop](https://docs.langchain.com/oss/python/deepagents/human-in-the-loop) for handling and resuming interrupts.

Filesystem permission interrupts require `deepagents>=0.6.8`.

### [​](https://docs.langchain.com/oss/python/deepagents/skills#allow-agents-to-edit-personal-skills) Allow agents to edit personal skills
...
3. Do not add a `deny` rule for the writable path. Place more specific rules before broader deny rules if you mix shared and personal paths ([rule ordering](https://docs.langchain.com/oss/python/deepagents/permissions#rule-ordering)).
...
The agent uses `write_file` and `edit_file` to create or update `SKILL.md` and supporting files under the writable path. To capture general learnings outside the skills format, route a separate path such as `/memories/` to another writable backend. See [Backends](https://docs.langchain.com/oss/python/deepagents/backends) for routing and store setup.

[​](https://docs.langchain.com/oss/python/deepagents/skills#execute-code-with-skills) Execute code with skills
--------------------------------------------------------------------------------------------------------------
...
Skills execute code through [sandbox scripts](https://docs.langchain.com/oss/python/deepagents/skills#sandbox-scripts): the agent runs a bundled script when it needs to install dependencies, run tests, call CLIs, or work with an operating-system filesystem.

### [​](https://docs.langchain.com/oss/python/deepagents/skills#sandbox-scripts) Sandbox scripts
...
The agent can *read* scripts from any backend, but to *execute* them, the agent needs access to a shell, which only [sandbox backends](https://docs.langchain.com/oss/python/deepagents/sandboxes) provide.
[Sandbox backends](https://docs.langchain.com/oss/python/deepagents/sandboxes) run in isolated containers. Skill files stored outside the sandbox are not available inside it, which means the agent cannot execute skill scripts or access skill resources unless they are transferred in first. Use [custom middleware](https://docs.langchain.com/oss/python/langchain/middleware/custom) to handle this transfer:
...
For a complete example that seeds both skills and memories before execution and syncs both back afterward, see [syncing skills and memories with custom middleware](https://docs.langchain.com/oss/python/deepagents/going-to-production#example-syncing-skills-and-memories-with-custom-middleware).

[​](https://docs.langchain.com/oss/python/deepagents/skills#troubleshooting) Troubleshooting
--------------------------------------------------------------------------------------------
...
### [​](https://docs.langchain.com/oss/python/deepagents/skills#skill-not-activated) Skill not activated

**Problem**: The agent handles the task without reading the skill’s `SKILL.md`.
**Solutions**:

1. **Make the description more specific.** The agent selects skills from the [`description`](https://docs.langchain.com/oss/python/deepagents/skills#frontmatter-fields) field alone at [discovery](https://docs.langchain.com/oss/python/deepagents/skills#how-skills-work). Include what the skill does, when to use it, and keywords the agent can match:
   ...
   ```
2. **Reduce overlap between skills.** If multiple skills have similar descriptions, the agent may skip the right one or pick the wrong one. Differentiate descriptions or [consolidate related skills](https://docs.langchain.com/oss/python/deepagents/skills#write-effective-skills).
...
### [​](https://docs.langchain.com/oss/python/deepagents/skills#skills-missing-at-startup) Skills missing at startup
...
2. **Validate `SKILL.md` [frontmatter](https://docs.langchain.com/oss/python/deepagents/skills#frontmatter-fields).** The [`name`](https://docs.langchain.com/oss/python/deepagents/skills#frontmatter-fields) must match the parent directory name and follow the [Agent Skills specification](https://agentskills.io/specification). Use the [`skills-ref` validation tool](https://github.com/agentskills/agentskills/tree/main/skills-ref) to check formatting.
3. **Check file size.** Deep Agents skips `SKILL.md` files over 10 MB during discovery.
4. **Check layered sources.** When the same skill name appears in multiple sources, the [last source wins](https://docs.langchain.com/oss/python/deepagents/skills#usage). An older or empty skill from a later path can override the one you expect.

### [​](https://docs.langchain.com/oss/python/deepagents/skills#supporting-files-not-found) Supporting files not found

**Problem**: The agent reads `SKILL.md` but cannot access scripts, references, or assets.
**Solutions**:

1. **Reference files from `SKILL.md`.** The agent does not auto-discover supporting files. State what each file contains and when to use it. Use [relative paths](https://docs.langchain.com/oss/python/deepagents/skills#reference-files-from-skill-md) from the skill root.
...
3. **Sync skills into sandboxes.** If you use [sandbox backends](https://docs.langchain.com/oss/python/deepagents/sandboxes), skill files outside the container are not available until you copy them in. See [Sandbox scripts](https://docs.langchain.com/oss/python/deepagents/skills#sandbox-scripts) and [syncing skills and memories with custom middleware](https://docs.langchain.com/oss/python/deepagents/going-to-production#example-syncing-skills-and-memories-with-custom-middleware).

### [​](https://docs.langchain.com/oss/python/deepagents/skills#scripts-fail-to-run) Scripts fail to run

**Problem**: The agent reads a script but cannot run it.
**Solution**: The agent can read scripts from any backend, but running them requires a [sandbox backend](https://docs.langchain.com/oss/python/deepagents/sandboxes). See [Execute code with skills](https://docs.langchain.com/oss/python/deepagents/skills#execute-code-with-skills).

### [​](https://docs.langchain.com/oss/python/deepagents/skills#subagent-cannot-access-a-skill) Subagent cannot access a skill

**Problem**: A custom subagent does not see skills that the main agent uses.
**Solution**: Custom subagents do not inherit the main agent’s skills. Add a `skills` parameter to each [subagent definition](https://docs.langchain.com/oss/python/deepagents/skills#skills-for-subagents) with that subagent’s skill source paths. The general-purpose subagent inherits skills from `create_deep_agent` automatically.

[​](https://docs.langchain.com/oss/python/deepagents/skills#reference) Reference
--------------------------------------------------------------------------------

### [​](https://docs.langchain.com/oss/python/deepagents/skills#skills-memory-and-tools) Skills, memory, and tools

Skills, [memory](https://docs.langchain.com/oss/python/deepagents/memory) (`AGENTS.md` files), and tools all provide context or capabilities to the agent. The following table summarizes when to reach for each:
...
### [​](https://docs.langchain.com/oss/python/deepagents/skills#frontmatter-fields) Frontmatter fields
...
Previous](https://docs.langchain.com/oss/python/deepagents/memory)[Sandboxes

Next](https://docs.langchain.com/oss/python/deepagents/sandboxes)
