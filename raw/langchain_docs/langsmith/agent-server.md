---
title: Agent Server
source: https://docs.langchain.com/langsmith/agent-server.md
---
...
Each worker handles up to [`N_JOBS_PER_WORKER`](/langsmith/env-var-cloud) runs concurrently (default: 10), so a single worker container serves many runs in parallel. See [Configure Agent Server for scale](/langsmith/agent-server-scale) for tuning guidance.
